...
struct TestObject {
    ...
    TestObject(TestObject&& other) noexcept : id(other.id), data(std::move(other.data)) {
...
TEST_CASE("MpscRingBuffer Construction and Capacity", "[ring_buffer]") {
    ...
    SECTION("Capacity is rounded to next power of two") {
        ...
        for (int i = 0; i < 4; ++i) REQUIRE(rb3.try_emplace(i));
        REQUIRE_FALSE(rb3.try_emplace(4));

        MpscRingBuffer<int> rb4(4); // next_power_of_two(4) is 4
        for (int i = 0; i < 4; ++i) REQUIRE(rb4.try_emplace(i));
        REQUIRE_FALSE(rb4.try_emplace(4));

        MpscRingBuffer<int> rb1(1); // next_power_of_two(1) is 2 (as per next_power_of_two impl for n=1 -> p=1, then p <<=1 -> p=2)
...
TEST_CASE("MpscRingBuffer Basic Operations", "[ring_buffer]") {
    ...
    SECTION("Try emplace on full buffer") {
        ...
        // Verify contents
        REQUIRE(rb.try_pop(temp)); REQUIRE(temp == 1);
        REQUIRE(rb.try_pop(temp)); REQUIRE(temp == 2);
        REQUIRE(rb.try_pop(temp)); REQUIRE(temp == 3);
        REQUIRE(rb.try_pop(temp)); REQUIRE(temp == 100);
...
TEST_CASE("MpscRingBuffer Wrap Around Behavior", "[ring_buffer]") {
    ...
    for(int iter = 0; iter < 3; ++iter) { // Repeat a few times
        for(int i = 0; i < 4; ++i) REQUIRE(rb_large.try_emplace(iter * 100 + i));
        for(int i = 0; i < 2; ++i) {
            int v;
            REQUIRE(rb_large.try_pop(v));
            REQUIRE(v == iter * 100 + i);
        }
         for(int i = 4; i < 6; ++i) REQUIRE(rb_large.try_emplace(iter * 100 + i)); // Add 2 more
...
TEST_CASE("MpscRingBuffer Object Lifecycle and Move Semantics", "[ring_buffer]") {
    TestObject::reset_counts();
    {
        ...
        SECTION("Emplace and Pop TestObject") {
            ...
            REQUIRE(rb.try_pop(out_val)); // 1 move (from buffer to out_val), 1 destruction (in buffer)
    ...
    // Any remaining objects in the buffer should be destructed by the buffer's destructor.
    // If the section above ran fully, constructions should equal destructions.
    // If objects were left, ~MpscRingBuffer would call destructors.
    // For this specific section, we pop everything, so TestObject::destructions
    // should account for the objects popped. The buffer itself is empty on destruction.
    ...
    SECTION("Ensure objects are destructed when buffer is destroyed") {
        ...
        REQUIRE(TestObject::constructions == 2);
        REQUIRE(TestObject::destructions == 2); // Two objects destructed by ~MpscRingBuffer
    }
}

TEST_CASE("MpscRingBuffer Stress Test (Single Producer, Single Consumer Sequential)", "[ring_buffer][stress]") {
...
TEST_CASE("MpscRingBuffer Multi-Producer, Single-Consumer Threaded Test", "[ring_buffer][threaded]") {
    ...
    auto producer_task = [&](int producer_id) {
        ...
        for (int i = 0; i < items_per_producer; ++i) {
            ...
            while (!rb.try_emplace(value)) {
                // Spin/yield if buffer is full - in a real app, might use a condition variable or backoff
    ...
    for (int i = 0; i < num_producers; ++i) {
        producer_threads.emplace_back(producer_task, i + 1); // producer_id starting from 1
    }

    long consumed_sum = 0;
    int consumed_count = 0;
    std::vector<bool> consumed_flags(total_items + num_producers, false); // A bit oversized to be safe with 1-based producer_id * items_per_producer
    ...
    while (consumed_count < total_items) {
        ...
        if (rb.try_pop(val)) {
            ...
            // Values are expected to be unique in the range [items_per_producer, (num_producers+1)*items_per_producer -1]
            // if producer_id starts from 1.
            // For producer_id * items_per_producer + i, where producer_id is 1 to N, i is 0 to M-1
            // Smallest value: 1*items_per_producer + 0 = items_per_producer
            // Largest value: num_producers*items_per_producer + (items_per_producer-1)
