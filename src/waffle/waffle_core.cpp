...
void collect_attributes_into_array(
    ...
    auto process_one_arg = [&](const auto& arg) {
        ...
        } else if constexpr (std::is_same_v<std::decay_t<decltype(arg)>, CausedBy>) {
            // CausedBy is handled separately and extracted by parse_args_impl.
            // If it appears here, it's redundant or a usage error. Ignore for attribute collection.
        }
...
template<typename... AttrArgs>
Span Tracer::start_span(const StaticStringSource &name, Id parent, Id cause, AttrArgs&&... attr_args) {
  uint64_t new_id_val = _next_id.fetch_add(1, std::memory_order_relaxed);
  Id new_id = {new_id_val};
  // TODO: Critical: If parent.value is a valid span_id, trace_id should be the parent's trace_id,
  // not parent.value itself (unless parent is the root span).
  // This requires a way to get the parent's trace_id, e.g., via Waffle::context.
  // Keeping original logic for now to focus on the try_emplace fix.
  Id trace_id =
  ...
  collect_attributes_into_array(collected_attrs_array, actual_attr_count, std::forward<AttrArgs>(attr_args)...);
...
template<typename... AttrArgs>
Span Tracer::start_span(std::string_view name, Id parent, Id cause, AttrArgs&&... attr_args) {
  uint64_t new_id_val = _next_id.fetch_add(1, std::memory_order_relaxed);
  Id new_id = {new_id_val};
...
void Tracer::end_span(Id trace_id, Id span_id) {
  // For SPAN_END, parent_span_id in Tracelet context usually refers to the span that just ended,
  // and its "parent" for context restoration is its original parent.
  // However, the Tracelet structure has parent_span_id. For an END record, this might be kInvalidId
  // or the span_id itself. The current call passes kInvalidId.
  // The name_string_hash is 0 as SPAN_END doesn't have a name in the same way.
  _queue->try_emplace(get_timestamp(), trace_id, span_id, kInvalidId,
                      kInvalidId, 0, Tracelet::RecordType::SPAN_END);
}

template<typename... AttrArgs>
void Tracer::create_event(const StaticStringSource &name, Id parent, Id cause, AttrArgs&&... attr_args) {
  Id event_id = {_next_id.fetch_add(1)};
  // TODO: trace_id logic for events needs careful review. If parent is a span_id,
  // trace_id should be that span's trace_id.
  Id trace_id = (parent.value != kInvalidId.value) ? Id{parent.value} : event_id;
